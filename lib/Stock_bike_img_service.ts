...
export async function Stock_bike_img_list_rule_families(): Promise<Stock_bike_img_rule_family[]> {
  const rows = (await sql`
    ...
    left join stock_bike_img_rule_family_category fc on fc.stock_bike_img_rule_family_id = f.id
    left join stock_bike_img_family_bike_group g on g.stock_bike_img_rule_family_id = f.id
