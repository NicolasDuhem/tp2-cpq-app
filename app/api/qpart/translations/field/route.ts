...
import { translateCoreField, translateMetadataField } from '@/lib/qpart/translations/field-translation-service';
...
export async function POST(req: NextRequest) {
  ...
  const coreField = String(body.core_field ?? '').trim();
  ...
  if (!coreField && !Number.isFinite(metadataDefinitionId)) {
    return NextResponse.json({ error: 'Invalid metadata_definition_id' }, { status: 400 });
  }

  try {
    if (coreField) {
      if (coreField !== 'name' && coreField !== 'description') {
        return NextResponse.json({ error: 'Invalid core_field' }, { status: 400 });
      }
      const result = await translateCoreField({
        partId,
        coreField,
        fillMissingOnly,
      });
