...
import { resolveConfigurationReferenceLite } from '@/lib/cpq/runtime/configuration-references';
...
export async function POST(req: NextRequest) {
  ...
  try {
    const resolved = await resolveConfigurationReferenceLite(configurationReference, {
      traceId,
      route: '/api/cpq/retrieve-configuration',
    ...
    const startInput = {
      ...
      headerId: resolved.canonical_header_id ?? resolved.header_id ?? 'Simulator',
      detailId: resolved.canonical_detail_id ?? resolved.finalized_detail_id ?? '',
      sourceHeaderId: resolved.source_header_id ?? resolved.canonical_header_id ?? resolved.header_id ?? 'Simulator',
      sourceDetailId: resolved.source_detail_id ?? resolved.canonical_detail_id ?? '',
