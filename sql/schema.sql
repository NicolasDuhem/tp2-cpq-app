...
create table if not exists stock_bike_img_business_bike_type (
  id bigserial primary key,
  stock_bike_img_bike_type_key text not null unique,
  stock_bike_img_bike_type_name text not null unique,
  stock_bike_img_sort_order integer not null default 100,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create table if not exists stock_bike_img_business_bike_type_digit_map (
  id bigserial primary key,
  stock_bike_img_business_bike_type_id bigint not null references stock_bike_img_business_bike_type(id) on delete cascade,
  stock_bike_img_digit_position integer not null,
  stock_bike_img_digit_value text not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint stock_bike_img_bike_type_digit_map_position_chk check (stock_bike_img_digit_position between 1 and 30),
  constraint stock_bike_img_bike_type_digit_map_digit_chk check (length(trim(stock_bike_img_digit_value)) > 0),
  constraint stock_bike_img_bike_type_digit_map_unique unique (stock_bike_img_digit_position, stock_bike_img_digit_value)
);

create index if not exists stock_bike_img_bike_type_digit_map_lookup_idx
  on stock_bike_img_business_bike_type_digit_map (stock_bike_img_digit_position, stock_bike_img_digit_value);

create table if not exists stock_bike_img_rule_family (
  id bigserial primary key,
  stock_bike_img_family_key text not null unique,
  stock_bike_img_family_name text not null unique,
  stock_bike_img_family_description text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create table if not exists stock_bike_img_rule_family_category (
  id bigserial primary key,
  stock_bike_img_rule_family_id bigint not null references stock_bike_img_rule_family(id) on delete cascade,
  stock_bike_img_rule_category_name text not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint stock_bike_img_rule_family_category_unique unique (stock_bike_img_rule_family_id, stock_bike_img_rule_category_name)
);

create table if not exists stock_bike_img_family_bike_group (
  id bigserial primary key,
  stock_bike_img_rule_family_id bigint not null references stock_bike_img_rule_family(id) on delete cascade,
  stock_bike_img_group_key text not null,
  stock_bike_img_group_name text not null,
  stock_bike_img_group_description text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint stock_bike_img_family_bike_group_unique unique (stock_bike_img_rule_family_id, stock_bike_img_group_key),
  constraint stock_bike_img_family_bike_group_name_unique unique (stock_bike_img_rule_family_id, stock_bike_img_group_name)
);

create table if not exists stock_bike_img_family_bike_group_member (
  id bigserial primary key,
  stock_bike_img_family_bike_group_id bigint not null references stock_bike_img_family_bike_group(id) on delete cascade,
  stock_bike_img_business_bike_type_id bigint not null references stock_bike_img_business_bike_type(id) on delete cascade,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint stock_bike_img_family_bike_group_member_unique unique (
    stock_bike_img_family_bike_group_id,
...
create table if not exists stock_bike_img_rule (
  id bigserial primary key,
  stock_bike_img_model_year integer,
  stock_bike_img_rule_category text not null,
  stock_bike_img_rule_family_id bigint not null references stock_bike_img_rule_family(id),
  stock_bike_img_bike_type_group_id bigint references stock_bike_img_family_bike_group(id),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint stock_bike_img_rule_model_year_chk check (stock_bike_img_model_year is null or stock_bike_img_model_year between 2020 and 2028),
...
alter table if exists stock_bike_img_rule
  alter column stock_bike_img_model_year drop not null;

alter table if exists stock_bike_img_rule
  add column if not exists stock_bike_img_rule_family_id bigint references stock_bike_img_rule_family(id);

alter table if exists stock_bike_img_rule
  add column if not exists stock_bike_img_bike_type_group_id bigint references stock_bike_img_family_bike_group(id);

update stock_bike_img_rule
set stock_bike_img_model_year = null
where stock_bike_img_model_year = 0;

do $$
begin
  if exists (
    select 1 from pg_constraint where conname = 'stock_bike_img_rule_unique_signature'
  ) then
    alter table stock_bike_img_rule drop constraint stock_bike_img_rule_unique_signature;
  end if;
end$$;

drop index if exists stock_bike_img_rule_unique_signature_idx;

create unique index if not exists stock_bike_img_rule_unique_signature_idx
  on stock_bike_img_rule (
    coalesce(stock_bike_img_model_year, 0),
    stock_bike_img_rule_category,
    stock_bike_img_rule_family_id,
    coalesce(stock_bike_img_bike_type_group_id, 0),
    stock_bike_img_conditions_signature
  );

create index if not exists stock_bike_img_rule_runtime_idx
  on stock_bike_img_rule (stock_bike_img_is_active, stock_bike_img_model_year, stock_bike_img_rule_family_id, stock_bike_img_bike_type_group_id, stock_bike_img_layer_order, id);

create index if not exists stock_bike_img_rule_category_idx
  on stock_bike_img_rule (stock_bike_img_rule_category, stock_bike_img_model_year, stock_bike_img_rule_family_id, stock_bike_img_layer_order, id);
